...
class ScoreBoard:
    ...
    def issue_stage(self):
        ...
        if len(self.fetch) == 0:
            ...
            if not res and len(next_instr.struct_hazard) == 0:
                next_instr.struct_hazard.append(self.chip.cpu.curr_clock)
            if not res and len(next_instr.waw_hazard) == 0:
    ...
    def exec_stage(self):
        ...
        for i, next_instr in enumerate(self.read):
            ...
            if next_instr.processing_unit == FPType.FPAdder:
                occupied_adders = [i for i, adder in enumerate(
                    self.chip.cpu.fp_adder) if adder[1]]
                print("Occupied headers: ", occupied_adders)
            ...
            if next_instr.processing_unit == FPType.FPDiv:
            ...
            if next_instr.processing_unit == FPType.FPMul:
    ...
    def write_stage(self):
        ...
        while self.exec:
            ...
            print("~~~~~ Finished Instruction {} score: {}".format(
                next_instr.print_instr(is_print=False), next_instr.res))
    ...
    def __assign_processing_unit(self, next_instr):
        ...
        if pu_type == FPType.IntALU:
            if not self.chip.cpu.int_alu[0][1] and not self.check_waw_hazards(next_instr):
                self.chip.cpu.int_alu[0][0].set_instruction(next_instr)
                self.chip.cpu.int_alu[0] = (self.chip.cpu.int_alu[0][0], True)
                res = True
            else:
